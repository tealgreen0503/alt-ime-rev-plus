...
alt-ime-rev-plus はデバッグモード ON 中の IME 操作を詳細ログとして記録します。
...
1. alt-ime-rev-plus を起動(タスクトレイに常駐)
...
```
(alt-ime-rev-plus-x64.exe と同じフォルダ)/
├── alt-ime-rev-plus-x64.exe
...
4. alt-ime-rev-plus を起動し、`Ctrl+Shift+F12` でデバッグモード ON
...
- タスクトレイに alt-ime-rev-plus アイコンが表示されていますか?
...
- alt-ime-rev-plus 側でデバッグモード ON になっていますか?

**Q: ログファイルはどこにある?**
- .exe と同じフォルダです(`alt-ime-rev-plus-x64.exe` の隣)
